...
### Phase A — temporary HTTP-only config

Replace the contents of `/opt/shelf/nginx/conf.d/shelf.conf` with just the HTTP block for now:
...
server {
    ...
    location / {
        return 200 'Shëlf — SSL setup in progress';
...
### Phase C — switch to the full Nginx config

Now replace `/opt/shelf/nginx/conf.d/shelf.conf` with the full config from step 10 (both the HTTP redirect block and the full HTTPS block).

...
export async function load({ cookies, locals }: RequestEvent) {
  ...
  const [
    ...
    supabase
      .from("sales")
      .select(
        "id, total, payment_method, created_at, customer:customers(id, name)",
      )
      .eq("shop_id", shopId)
      .is("voided_at", null)
    ...
    supabase
      .from("sale_items")
      .select(
        "unit_price, qty, line_total, cost_at_sale, sale:sales!inner(shop_id, voided_at, created_at), product:products(id, name, cost_price, category:categories(id, name, color, icon))",
      )
    ...
    supabase
      .from("sale_items")
      .select(
        "unit_price, qty, cost_at_sale, sale:sales!inner(shop_id, voided_at, created_at), product:products(cost_price)",
      )
      .eq("sale.shop_id", shopId)
      .is("sale.voided_at", null)
    ...
    supabase
      .from("products")
      .select(
        "id, name, price, cost_price, qty, category:categories(id, name, color, icon)",
      )
      .eq("shop_id", shopId)
      .is("archived_at", null)
      .limit(1000),
  ...
  // ── Today ────────────────────────────────────────────────────────────────
  const todayRevenue = ((todaySales as any[]) ?? []).reduce(
    (s, x) => s + x.total,
    0,
  );
  ...
  const todayCost = ((saleItemsToday as any[]) ?? []).reduce(
    (s, item) =>
      s + (item.cost_at_sale ?? item.product?.cost_price ?? 0) * item.qty,
    0,
  );
  ...
  // ── Yesterday ────────────────────────────────────────────────────────────
  const yestRevenue = ((yestSales as any[]) ?? []).reduce(
    (s, x) => s + x.total,
    0,
  );
  ...
  const paymentBreakdown = ((todaySales as any[]) ?? []).reduce(
    (acc, sale) => {
  ...
  const productAgg = new Map<
    string,
    { id: string; name: string; qty: number; revenue: number; category: any }
  >();
  ...
  const totalItemsToday = ((saleItemsToday as any[]) ?? []).reduce(
    (s, x) => s + x.qty,
    0,
  );
  const avgBasket =
    todayCount > 0 ? +(totalItemsToday / todayCount).toFixed(1) : 0;

...
interface ShelfDB extends DBSchema {
 ...
 /** Cached analytics response, keyed by period hash. */
 analytics_cache: {
  key: string; // period hash (e.g. '7d' or '2024-01-01_2024-01-31')
  value: {
   key: string;
   data: any; // the full analytics response
   cached_at: number;
  };
  indexes: { "by-cached": number };
 };
 ...
 _db = openDB<ShelfDB>(DB_NAME, DB_VERSION, {
  upgrade(db, oldVersion) {
   ...
   if (oldVersion < 4) {
    ...
    const saleItems = db.createObjectStore("sale_items", {
     keyPath: "sale_id",
    });
    saleItems.createIndex("by-cached", "cached_at");
   }
   if (oldVersion < 5) {
    // v5: cached analytics responses (keyed by period hash)
    const analyticsCache = db.createObjectStore("analytics_cache", {
     keyPath: "key",
    });
    analyticsCache.createIndex("by-cached", "cached_at");
   }

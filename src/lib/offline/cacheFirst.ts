...
export async function cacheFirst<T>(
  ...
  const { maxAge = DEFAULT_MAX_AGE, alwaysRefresh = false, label = storeName } = options;
  const now = Date.now();
  ...
  if (offlineSync.online) {
    try {
      ...
      throw new Error(`Cache cold and network fetch failed for ${label}: ${(err as Error).message}`);
    }
  }
...
export async function optimisticWrite<T>(
  ...
  value: T,
  syncFn: () => Promise<void>,
  options: { label?: string; onSyncFailure?: (err: Error) => void } = {},
): Promise<{ written: boolean; syncing: boolean }> {
  const { label = storeName } = options;
 ...
 */
export async function invalidateCache(storeName: string, key?: string): Promise<void> {
  if (!browser) return;
  if (key) {
    await deleteCache(storeName, key);
  } else {

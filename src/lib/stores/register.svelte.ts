...
class RegisterStore {
  ...
  get balance() {
    ...
    return {
      destinations: Object.entries(dests).map(([destination, balance]) => ({
        destination,
        balance,
      })),
      total,
  ...
  get counterBalance() {
    return (
      this.balance.destinations.find((d) => d.destination === "counter")
        ?.balance ?? 0
    );
  }
  ...
  reconcile(clientId: string, real: RegisterEntry) {
    this.#entries = this.#entries.map((e) =>
      (e as any).client_id === clientId ? real : e,
    );
  }
  ...
  markSynced(id: string) {
    this.#entries = this.#entries.map((e) =>
      e.id === id
        ? (() => {
            const { _pending, _local, ...rest } = e as any;
            return rest as RegisterEntry;
          })()
        : e,
    );
  }

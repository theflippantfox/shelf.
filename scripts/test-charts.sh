...
rm -f /tmp/c.jar
curl -sS -c /tmp/c.jar -X POST http://localhost:5174/api/auth/register \
  -H "Content-Type: application/json" \
  -d "{\"email\":\"$EMAIL\",\"password\":\"$PASSWORD\",\"first_name\":\"C\",\"last_name\":\"H\"}" > /dev/null
curl -sS -b /tmp/c.jar -c /tmp/c.jar -X POST http://localhost:5174/api/auth \
  -H "Content-Type: application/json" -d "{\"email\":\"$EMAIL\",\"password\":\"$PASSWORD\"}" > /dev/null
curl -sS -b /tmp/c.jar -X POST http://localhost:5174/api/onboarding/shop \
  -H "Content-Type: application/json" -d "{\"name\":\"CH Shop\",\"slug\":\"$SLUG\"}" > /dev/null
curl -sS -b /tmp/c.jar -X POST http://localhost:5174/api/onboarding/locale \
  -H "Content-Type: application/json" \
...
curl -sS -b /tmp/c.jar -X POST http://localhost:5174/api/onboarding/appearance \
  -H "Content-Type: application/json" -d '{"theme":"system","primary_color":"#7B4F8A","sidebar_bg":"#150F1C"}' > /dev/null
curl -sS -b /tmp/c.jar -X POST http://localhost:5174/api/onboarding/team \
  -H "Content-Type: application/json" -d '{"invites":[]}' > /dev/null
curl -sS -b /tmp/c.jar -X POST http://localhost:5174/api/onboarding/categories \
  -H "Content-Type: application/json" -d '{"categories":[]}' > /dev/null

# A product
P1=$(curl -sS -b /tmp/c.jar -X POST http://localhost:5174/api/products \
  -H "Content-Type: application/json" \
...
for i in 1 2 3 4 5; do
  curl -sS -b /tmp/c.jar -X POST http://localhost:5174/api/sales \
    -H "Content-Type: application/json" \
...
echo "=== Page /analytics ==="
CODE=$(curl -sS -b /tmp/c.jar -o /tmp/analytics.html -w "%{http_code}" "http://localhost:5174/analytics")
echo "  HTTP $CODE"
if [ "$CODE" = "200" ]; then
...
echo ""
echo "=== Page / ==="
CODE=$(curl -sS -b /tmp/c.jar -o /tmp/home.html -w "%{http_code}" "http://localhost:5174/")
echo "  HTTP $CODE"